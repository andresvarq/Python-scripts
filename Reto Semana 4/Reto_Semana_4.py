def promedio_facultades(info: dict, contando_externos : bool = True ) -> tuple:
	#eliminar materias retiradas y sin creditos
	for i in info:
		j = 0
		while j < len(info[i]['materias']):
			if info[i]['materias'][j]['creditos'] == 0 or info[i]['materias'][j]['retirada'] == 'Si':
				info[i]['materias'][j] = 0
			j += 1
		while 0 in info[i]['materias']:
			info[i]['materias'].remove(0)

	#eliminar electivas y materias de verano cuando contando_externos == False
	if contando_externos == False:
		for i in info.copy():
			for j in range(len(info[i]['materias'])):
				while info[i]['materias'][j]['codigo'][-1] != '-':
					info[i]['materias'][j]['codigo'] = info[i]['materias'][j]['codigo'].rstrip(info[i]['materias'][j]['codigo'][-1])
				info[i]['materias'][j]['codigo'] = info[i]['materias'][j]['codigo'].rstrip(info[i]['materias'][j]['codigo'][-1])
				if (info[i]['materias'][j]['codigo'] != info[i]['programa']):
					info[i]['materias'][j] = 0
			while 0 in info[i]['materias']:
				info[i]['materias'].remove(0)
			if (str(i)[4:6] == '05'):
	...
	#Eliminar información de estudiantes no utilizados
	for i in info.copy():
		if info[i]['materias'] == []:
			info.pop(i)
	...
	for i in info.values():
		apellidos = i['apellidos'].split()
		apellidos[0], apellidos[1] = apellidos[1], apellidos[0]
		nombreCompleto = (i['nombres'] + ' ').split()
		nombreCompleto = nombreCompleto + apellidos
		documento = str(i['documento'])
		correo = (nombreCompleto[0][0] + nombreCompleto[1][0] + '.' + nombreCompleto[2] + documento[-2] + documento[-1]).lower()
		for j in correo:
			if j == 'á':
				correo = correo.replace(j, 'a')
			elif j == 'é':
				correo = correo.replace(j, 'e')
			elif j == 'í':
				correo = correo.replace(j, 'i')
			elif j == 'ó':
				correo = correo.replace(j, 'o')
			elif j == 'ú':
				correo = correo.replace(j, 'u')
			elif j == 'ñ':
				correo = correo.replace(j, 'n')
			elif j == ',':
				correo = correo.replace(j, '')
		correos.append(correo)
	correos_2 = set(correos)
	correos = list(correos_2)
	correos.sort()
	...
	try:
		...
		for key, value in info.items():
			...
			divisor = 0
			dividendo = 0
			while a < len(materias):
				facultad = materias[a]['facultad']
				nota = materias[a]['nota']
				creditos = materias[a]['creditos']
				materia = facultad, nota, creditos,
				facultades.append(materia)
				a += 1
		facultades.sort()
		...
		divisor = 0
		dividendo = 0
		while a < len(facultades) -1:
			b = a + 1
			if facultades[a][0] == facultades[b][0]:
				divisor = divisor + (facultades[a][1] * facultades[a][2])
				dividendo = dividendo + facultades[a][2]
			elif facultades[a][0] == facultades[b-1][0]:
				divisor = divisor + (facultades[a][1] * facultades[a][2])
				dividendo = dividendo + facultades[a][2]
				promedio[facultades[a][0]] = round((divisor / dividendo), 2)
				divisor = 0
				dividendo = 0
			else:
				promedio[facultades[a][0]] = round((divisor / dividendo), 2)
